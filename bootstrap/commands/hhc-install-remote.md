...
6. Model keşfi klonlanan proje dizini için `model_discovery.py --project-path <hedef>` ile yapılır.
7. Tek Ana Ajanda bir model bütün rollere; Çoklu Ajanda doğrudan rol → model.
8. Son özet ve onay.
...
Backend:
`{{PYTHON}} "{{KIT_ROOT}}/scripts/remote_install.py" --repo "$ARGUMENTS" --team-mode <single|multi> --preset <profil> [--manager-mode <mod>] [--roles ...] [--shared-model provider/model | --model role=provider/model ...] --scout <enabled|disabled> [--scout-model provider/model]`
